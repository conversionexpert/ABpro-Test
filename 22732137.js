...
(function(){
	...
	var Script = {
		...
		init: function(projectData){
			...
			//console.log(this.projectData); //DEBUG
		...
		carriesExperiment: function(experimentIdLookup){
			...
			var experiments = this.projectData.experiments;
			for(var index in experiments){
				if(experiments[index].id === experimentIdLookup){
					return true;
				}
			}
			return false;
    ...
    var Project = {
    	...
    	init: function(data){
    		...
    		console.groupCollapsed('ABpro # Initiated PROJECT', this.data.id);
    ...
    Experiment.prototype.passesExclusionGroups = function(){
    	var currentExperimentId = this.data.id;
    	var passed = true;
...
//			console.log(exclusionGroupExperimentIds, currentExperimentId);
//			console.log( exclusionGroupExperimentIds.indexOf(currentExperimentId) );
			
			//Check if the current Experiment is in the exclusionGroup - if not, skip that group
			if(exclusionGroupExperimentIds.indexOf(currentExperimentId) === -1){
				return;
			}

			//Look for already bucketed Experiments
			for(var experimentsBucketExperimentId in User.experimentsBucket){
				experimentsBucketExperimentId = parseInt(experimentsBucketExperimentId);
				
//				console.log(experimentsBucketExperimentId);
				
				
				
				//Skip if the current Experiment ID from the Bucket is not in the Exclusion Group
				if(exclusionGroupExperimentIds.indexOf(experimentsBucketExperimentId)){
					return;
				}
				
				

	  			//var experimentsBucketExperimentVariationId = false;
				//if(User.experimentsBucket.hasOwnProperty(experimentsBucketExperimentId)){
				experimentsBucketExperimentVariationId = User.experimentsBucket[experimentsBucketExperimentId];
				//}
				
				//Check if...
				//the Experiment is still (active/carried) in the Snippet
				//and the User wasnt excluded => VariationId = 0
				if(Script.carriesExperiment(experimentsBucketExperimentId)){
					if(experimentsBucketExperimentVariationId){
						exclusionGroupPickedExperimentId = experimentsBucketExperimentId;
					}
				}
			...
			//console.log(exclusionGroupPickedExperimentId); //PROBLEM: Immer eine ID, nie FALSE 
			
			
			
			//When User is not already bucketed, choose a random Experiment ID from Exclusion Group
			if(!exclusionGroupPickedExperimentId){
				
				//Get a random ExperimentId with given Probability
		    	var experimentsDicerArr = [];
		    	var dicedExperimentId = false;  			
		  		for(var exclusionGroupEntryKey in exclusionGroupEntries){
		  			exclusionGroupEntry = exclusionGroupEntries[exclusionGroupEntryKey];
					for(i = 0; i < exclusionGroupEntry.traffic; i++){
						experimentsDicerArr.push(exclusionGroupEntry.experiment_id);
					}
				}
		  		
		  		//Fill to 100 (with false)
		  		while(experimentsDicerArr.length < 100){
		  			experimentsDicerArr.push(false);
		  		}
		  		
				var chosenExclusionGroupEntryKey = Math.floor(Math.random() * experimentsDicerArr.length);
				dicedExperimentId = exclusionGroupEntry[chosenExclusionGroupEntryKey];

				//Set Experiment ID
				//exclusionGroupPickedExperimentId = exclusionGroupExperimentIds[ Math.floor(Math.random() * exclusionGroupExperimentIds.length) ];
				exclusionGroupPickedExperimentId = dicedExperimentId;
			}
			
//			console.log(exclusionGroupPickedExperimentId, currentExperimentId);
			
			if(exclusionGroupPickedExperimentId !== currentExperimentId){
				passed = false; //TODO BUG
			}
    ...
    Script.init({"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}","experiments":[{"id":16552854,"status":"running","name":"Exclusion Experiment 3","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp3');","css":"","audiences":[{"id":49788342,"name":"Desktop Users","rules_js":"if(User.browser.desktop){\r\n  return true;\r\n}\r\n"}],"variations":[{"id":37655941,"name":"Original","baseline":1,"traffic_allocation":50,"js":"","css":"","experiment":{"id":16552854,"status":"running","name":"Exclusion Experiment 3","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp3');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}},{"id":60098426,"name":"Variation #1","baseline":0,"traffic_allocation":50,"js":"","css":"","experiment":{"id":16552854,"status":"running","name":"Exclusion Experiment 3","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp3');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}}],"pages":[{"id":59968434,"name":"Index","rules_js":"return window.testVar;","pivot":{"experiment_id":16552854,"page_id":59968434},"urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}],"goals":[],"project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}},{"id":40015670,"status":"running","name":"Anian Exp 1","traffic_allocation":100,"audiences_match_type":"all","js":"","css":"","audiences":[{"id":56425134,"name":"Dummy Audience","rules_js":"return false;"}],"variations":[{"id":62262916,"name":"Original","baseline":1,"traffic_allocation":50,"js":"","css":"","experiment":{"id":40015670,"status":"running","name":"Anian Exp 1","traffic_allocation":100,"audiences_match_type":"all","js":"","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}},{"id":60923097,"name":"Variation #1","baseline":0,"traffic_allocation":50,"js":"","css":"","experiment":{"id":40015670,"status":"running","name":"Anian Exp 1","traffic_allocation":100,"audiences_match_type":"all","js":"","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}}],"pages":[{"id":59968434,"name":"Index","rules_js":"return window.testVar;","pivot":{"experiment_id":40015670,"page_id":59968434},"urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}],"goals":[{"id":47363538,"type":"custom","counting_method":"one","css_selector":null,"name":"Test Goal 1","api_name":"Test Goal 1","rules_js":null,"pivot":{"experiment_id":40015670,"goal_id":47363538},"page":null}],"project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}},{"id":49616615,"status":"running","name":"Headlines with direct Speech","traffic_allocation":100,"audiences_match_type":"any","js":"\/\/https:\/\/solaranlagen.firmen.de\/start_1480.html \r\n\/\/https:\/\/photovoltaikanlagen.firmen.de\/start_1576.html\r\n\r\n(function(){\r\n\r\n    \/* + + + + + + + + + + ArTools + + + + + + + + + + *\/\r\n\r\n\tvar ArTools = {\r\n\r\n\t\tinit: function(){\r\n\t\t\tvar _self = this;\r\n\t\t\twindow.addEventListener('load', function(){\r\n        \t\t_self.runVariationCallback();\r\n        \t}, false);\r\n\t\t},\r\n\r\n        runVariationCallback: function(){\r\n            ArVariation.init();\r\n        },\r\n\r\n\t\tsetCookie: function(name, value, days){\r\n\t\t\tvar expires = \"\";\r\n\t\t\tif(days){\r\n\t\t\t\tvar date = new Date();\r\n\t\t\t\tdate.setTime(date.getTime()+(days*24*60*60*1000));\r\n\t\t\t\texpires = \"; expires=\"+date.toGMTString();\r\n\t\t\t}\r\n\t\t\tdocument.cookie = name+\"=\"+value+expires+\"; path=\/\";\r\n\t\t},\r\n\r\n\t\tgetCookie: function(name){\r\n\t\t\tvar nameEQ = name + \"=\";\r\n\t\t\tvar ca = document.cookie.split(';');\r\n\t\t\tfor(var i=0;i < ca.length;i++) {\r\n\t\t\t\tvar c = ca[i];\r\n\t\t\t\twhile (c.charAt(0)===' '){\r\n\t\t\t\t\tc = c.substring(1,c.length);\r\n\t\t\t\t}\r\n\t\t\t\tif (c.indexOf(nameEQ) === 0){\r\n\t\t\t\t\treturn c.substring(nameEQ.length,c.length);\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\treturn null;\r\n\t\t},\r\n\r\n\t\tremoveCookie: function(name){\r\n  \t\t\tdocument.cookie = name +'=; Path=\/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';\r\n\t\t},\r\n\r\n\t\tgetUrlParameter: function(name, url){\r\n\t\t    if(!url){\r\n\t\t      url = window.location.href;\r\n\t\t    }\r\n\t\t    name = name.replace(\/[\\[\\]]\/g, \"\\\\$&\");\r\n\t\t    var regex = new RegExp(\"[?&]\" + name + \"(=([^&#]*)|&|#|$)\"),\r\n\t\t        results = regex.exec(url);\r\n\t\t    if(!results){\r\n\t\t        return null;\r\n\t\t    }\r\n\t\t    if(!results[2]){\r\n\t\t        return '';\r\n\t\t    }\r\n\t\t    return decodeURIComponent(results[2].replace(\/\\+\/g, \" \"));\r\n\t\t},\r\n\r\n\t};\r\n\r\n\tArTools.init();\r\n\r\n\r\n    \/* + + + + + + + + + + ArVariation + + + + + + + + + + *\/\r\n\r\n    var ArVariation = {\r\n\r\n        init: function(){\r\n\t\t\tthis.addHelpers();\r\n\t\t\tthis.run();\r\n\t\t},\r\n\r\n        addHelpers: function(){\r\n            document.documentElement.classList.add('arExp_001__1');\r\n\t\t\tif(window.location.href.indexOf('datenEingabe') > -1){\r\n\t\t\t\tthis.pagetype = 'dateneingabe';\r\n\t\t       document.documentElement.classList.add('arPage_dateneingabe');\r\n\t\t    }\r\n\t\t\telse{\r\n\t\t\t\tthis.pagetype = 'default';\r\n\t\t\t\tdocument.documentElement.classList.add('arPage_default');\r\n\t\t\t}\r\n        },\r\n\r\n\t\trun: function(){\r\n\t\t\t\/\/General Alternation\r\n\t\t\tthis.alterStepPlz();\r\n\t\t\t\/\/Intent\r\n\t\t\tthis.setIntent();\r\n\t\t\tthis.intentId = this.getIntent();\r\n\t\t\tif(this.intentId){\r\n\t\t\t\tthis.changeElements();\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\t\/* - - - - - - - - - - Variation Functions - - - - - - - - - - *\/\r\n\r\n\t\tgetSubdomain: function(){\r\n\t\t\tvar hostname = window.location.hostname;\r\n\t\t\tvar regexParse = new RegExp('[a-z\\-0-9]{2,63}\\.[a-z\\.]{2,5}$');\r\n        \tvar urlParts = regexParse.exec(hostname);\r\n        \treturn hostname.replace(urlParts[0],'').slice(0, -1);\r\n\t\t},\r\n\r\n\t\tsetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getUrlParameter('i');\r\n\t\t\tif(intentId){\r\n\t\t\t\tArTools.setCookie('userintent', intentId);\r\n\t\t\t\tif(intentId === 'reset'){\r\n\t\t\t\t\tArTools.removeCookie('userintent');\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getCookie('userintent');\r\n\t\t\tif(intentId){\r\n\t\t\t\treturn intentId;\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\talterStepPlz: function(){\r\n\t\t\ttry {\r\n\t\t\t\tvar subdomain = this.getSubdomain();\r\n\t\t\t\tvar frageLiClass = 'frage8';\r\n\t\t\t\tif(subdomain === 'photovoltaikanlagen'){\r\n\t\t\t\t\tfrageLiClass = 'frage15';\r\n\t\t\t\t}\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass).classList.add('arStepPlz');\r\n\r\n\t\t\t\tvar plzCta = document.querySelector('li.'+frageLiClass+' .sliderNextWrap > button');\r\n\t\t\t\tplzCta.classList.add('arBtnNextPlz');\r\n\t\t\t\tplzCta.insertAdjacentHTML('afterbegin', '\u00bb ');\r\n\r\n\t\t\t\tvar plzErrorLabel = '<div class=\"arPlzErrorLabel\"><\/div>';\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass+' .sliderNextWrap').insertAdjacentHTML('afterend', plzErrorLabel);\r\n\r\n\t\t\t\tvar plzInput = document.querySelector('li.'+frageLiClass+' .sliderAnswerWrap input');\r\n\t\t\t\tplzCta.addEventListener('click', function(){\r\n\t\t\t\t\tsetTimeout(function(){\r\n\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '';\r\n\t\t\t\t\t\tif(plzInput.classList.contains('mfwError')){\r\n\t\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '<span><i class=\"fa fa-info-circle\" aria-hidden=\"true\"><\/i> Bitte geben Sie eine <strong>PLZ<\/strong> an, damit wir <strong>regionale Anbieter<\/strong> f\u00fcr Sie ermitteln k\u00f6nnen.<\/span>';\r\n\t\t\t\t\t\t}\r\n\t\t\t\t\t}, 25);\r\n\t\t\t\t});\r\n\t\t\t} catch(e){\r\n\t\t\t\t\/\/Nothing\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetContentArray: function(){\r\n\t\t\tvar subdomain = this.getSubdomain();\r\n\r\n\t\t\tvar content = {\r\n\r\n\t\t\t\t\/\/SOLARANLAGEN\r\n\t\t\t\tsolaranlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t\t\/\/PHOTOVOLTAIKANLAGEN\r\n\t\t\t\tphotovoltaikanlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Angebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Agebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t};\r\n\r\n\t\t\treturn content[subdomain];\r\n\t\t},\r\n\r\n\t\tgetContent: function(label){\r\n\t\t\tvar cid = this.intentId -1;\r\n\t\t\tvar content = this.getContentArray();\r\n\t\t\tif(content && cid in content){\r\n\t\t\t\treturn content[cid][label];\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\tchangeContent: function(element, content){\r\n\t\t\tif(element && content){\r\n\t\t\t\telement.innerHTML = content;\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tchangeElements: function(){\r\n\r\n\t\t\t\/\/GENERELL - - - - -\r\n\r\n\t\t\tvar d = document;\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.slogan'),\r\n\t\t\t\tthis.getContent('head_claim')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h1'),\r\n\t\t\t\tthis.getContent('slider_hl')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h2 > span'),\r\n\t\t\t\tthis.getContent('slider_sub_hl')\r\n\t\t\t);\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[0],\r\n\t\t\t\tthis.getContent('slider_uvp_1')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[1],\r\n\t\t\t\tthis.getContent('slider_uvp_2')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[2],\r\n\t\t\t\tthis.getContent('slider_uvp_3')\r\n\t\t\t);\r\n\r\n\r\n\t\t\t\/\/DATENEINGABE - - - - -\r\n\r\n\t\t\tif(this.pagetype === 'dateneingabe'){\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .col-head-form > .alert-success'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_notification')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .formular .col-head-form h3'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_intro')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tvar checkIcon = '<i class=\"glyphicon glyphicon-ok\"><\/i> ';\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[0],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_1')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[1],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_2')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[2],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_3')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tif(this.getContent('dateneingabe_cta')){\r\n\t\t\t\t\td.querySelector('#mfw_submit_adressInputStep1').setAttribute('value', this.getContent('dateneingabe_cta'));\r\n\t\t\t\t}\r\n\r\n\t\t\t}\r\n\r\n\t\t}\r\n\r\n    };\r\n\r\n})();\r\n","css":"","audiences":[{"id":49788342,"name":"Desktop Users","rules_js":"if(User.browser.desktop){\r\n  return true;\r\n}\r\n"},{"id":56425134,"name":"Dummy Audience","rules_js":"return false;"}],"variations":[{"id":72647771,"name":"Original","baseline":1,"traffic_allocation":49,"js":"","css":"body{\r\n\tborder: 5px solid red;\r\n}","experiment":{"id":49616615,"status":"running","name":"Headlines with direct Speech","traffic_allocation":100,"audiences_match_type":"any","js":"\/\/https:\/\/solaranlagen.firmen.de\/start_1480.html \r\n\/\/https:\/\/photovoltaikanlagen.firmen.de\/start_1576.html\r\n\r\n(function(){\r\n\r\n    \/* + + + + + + + + + + ArTools + + + + + + + + + + *\/\r\n\r\n\tvar ArTools = {\r\n\r\n\t\tinit: function(){\r\n\t\t\tvar _self = this;\r\n\t\t\twindow.addEventListener('load', function(){\r\n        \t\t_self.runVariationCallback();\r\n        \t}, false);\r\n\t\t},\r\n\r\n        runVariationCallback: function(){\r\n            ArVariation.init();\r\n        },\r\n\r\n\t\tsetCookie: function(name, value, days){\r\n\t\t\tvar expires = \"\";\r\n\t\t\tif(days){\r\n\t\t\t\tvar date = new Date();\r\n\t\t\t\tdate.setTime(date.getTime()+(days*24*60*60*1000));\r\n\t\t\t\texpires = \"; expires=\"+date.toGMTString();\r\n\t\t\t}\r\n\t\t\tdocument.cookie = name+\"=\"+value+expires+\"; path=\/\";\r\n\t\t},\r\n\r\n\t\tgetCookie: function(name){\r\n\t\t\tvar nameEQ = name + \"=\";\r\n\t\t\tvar ca = document.cookie.split(';');\r\n\t\t\tfor(var i=0;i < ca.length;i++) {\r\n\t\t\t\tvar c = ca[i];\r\n\t\t\t\twhile (c.charAt(0)===' '){\r\n\t\t\t\t\tc = c.substring(1,c.length);\r\n\t\t\t\t}\r\n\t\t\t\tif (c.indexOf(nameEQ) === 0){\r\n\t\t\t\t\treturn c.substring(nameEQ.length,c.length);\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\treturn null;\r\n\t\t},\r\n\r\n\t\tremoveCookie: function(name){\r\n  \t\t\tdocument.cookie = name +'=; Path=\/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';\r\n\t\t},\r\n\r\n\t\tgetUrlParameter: function(name, url){\r\n\t\t    if(!url){\r\n\t\t      url = window.location.href;\r\n\t\t    }\r\n\t\t    name = name.replace(\/[\\[\\]]\/g, \"\\\\$&\");\r\n\t\t    var regex = new RegExp(\"[?&]\" + name + \"(=([^&#]*)|&|#|$)\"),\r\n\t\t        results = regex.exec(url);\r\n\t\t    if(!results){\r\n\t\t        return null;\r\n\t\t    }\r\n\t\t    if(!results[2]){\r\n\t\t        return '';\r\n\t\t    }\r\n\t\t    return decodeURIComponent(results[2].replace(\/\\+\/g, \" \"));\r\n\t\t},\r\n\r\n\t};\r\n\r\n\tArTools.init();\r\n\r\n\r\n    \/* + + + + + + + + + + ArVariation + + + + + + + + + + *\/\r\n\r\n    var ArVariation = {\r\n\r\n        init: function(){\r\n\t\t\tthis.addHelpers();\r\n\t\t\tthis.run();\r\n\t\t},\r\n\r\n        addHelpers: function(){\r\n            document.documentElement.classList.add('arExp_001__1');\r\n\t\t\tif(window.location.href.indexOf('datenEingabe') > -1){\r\n\t\t\t\tthis.pagetype = 'dateneingabe';\r\n\t\t       document.documentElement.classList.add('arPage_dateneingabe');\r\n\t\t    }\r\n\t\t\telse{\r\n\t\t\t\tthis.pagetype = 'default';\r\n\t\t\t\tdocument.documentElement.classList.add('arPage_default');\r\n\t\t\t}\r\n        },\r\n\r\n\t\trun: function(){\r\n\t\t\t\/\/General Alternation\r\n\t\t\tthis.alterStepPlz();\r\n\t\t\t\/\/Intent\r\n\t\t\tthis.setIntent();\r\n\t\t\tthis.intentId = this.getIntent();\r\n\t\t\tif(this.intentId){\r\n\t\t\t\tthis.changeElements();\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\t\/* - - - - - - - - - - Variation Functions - - - - - - - - - - *\/\r\n\r\n\t\tgetSubdomain: function(){\r\n\t\t\tvar hostname = window.location.hostname;\r\n\t\t\tvar regexParse = new RegExp('[a-z\\-0-9]{2,63}\\.[a-z\\.]{2,5}$');\r\n        \tvar urlParts = regexParse.exec(hostname);\r\n        \treturn hostname.replace(urlParts[0],'').slice(0, -1);\r\n\t\t},\r\n\r\n\t\tsetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getUrlParameter('i');\r\n\t\t\tif(intentId){\r\n\t\t\t\tArTools.setCookie('userintent', intentId);\r\n\t\t\t\tif(intentId === 'reset'){\r\n\t\t\t\t\tArTools.removeCookie('userintent');\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getCookie('userintent');\r\n\t\t\tif(intentId){\r\n\t\t\t\treturn intentId;\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\talterStepPlz: function(){\r\n\t\t\ttry {\r\n\t\t\t\tvar subdomain = this.getSubdomain();\r\n\t\t\t\tvar frageLiClass = 'frage8';\r\n\t\t\t\tif(subdomain === 'photovoltaikanlagen'){\r\n\t\t\t\t\tfrageLiClass = 'frage15';\r\n\t\t\t\t}\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass).classList.add('arStepPlz');\r\n\r\n\t\t\t\tvar plzCta = document.querySelector('li.'+frageLiClass+' .sliderNextWrap > button');\r\n\t\t\t\tplzCta.classList.add('arBtnNextPlz');\r\n\t\t\t\tplzCta.insertAdjacentHTML('afterbegin', '\u00bb ');\r\n\r\n\t\t\t\tvar plzErrorLabel = '<div class=\"arPlzErrorLabel\"><\/div>';\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass+' .sliderNextWrap').insertAdjacentHTML('afterend', plzErrorLabel);\r\n\r\n\t\t\t\tvar plzInput = document.querySelector('li.'+frageLiClass+' .sliderAnswerWrap input');\r\n\t\t\t\tplzCta.addEventListener('click', function(){\r\n\t\t\t\t\tsetTimeout(function(){\r\n\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '';\r\n\t\t\t\t\t\tif(plzInput.classList.contains('mfwError')){\r\n\t\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '<span><i class=\"fa fa-info-circle\" aria-hidden=\"true\"><\/i> Bitte geben Sie eine <strong>PLZ<\/strong> an, damit wir <strong>regionale Anbieter<\/strong> f\u00fcr Sie ermitteln k\u00f6nnen.<\/span>';\r\n\t\t\t\t\t\t}\r\n\t\t\t\t\t}, 25);\r\n\t\t\t\t});\r\n\t\t\t} catch(e){\r\n\t\t\t\t\/\/Nothing\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetContentArray: function(){\r\n\t\t\tvar subdomain = this.getSubdomain();\r\n\r\n\t\t\tvar content = {\r\n\r\n\t\t\t\t\/\/SOLARANLAGEN\r\n\t\t\t\tsolaranlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t\t\/\/PHOTOVOLTAIKANLAGEN\r\n\t\t\t\tphotovoltaikanlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Angebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Agebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t};\r\n\r\n\t\t\treturn content[subdomain];\r\n\t\t},\r\n\r\n\t\tgetContent: function(label){\r\n\t\t\tvar cid = this.intentId -1;\r\n\t\t\tvar content = this.getContentArray();\r\n\t\t\tif(content && cid in content){\r\n\t\t\t\treturn content[cid][label];\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\tchangeContent: function(element, content){\r\n\t\t\tif(element && content){\r\n\t\t\t\telement.innerHTML = content;\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tchangeElements: function(){\r\n\r\n\t\t\t\/\/GENERELL - - - - -\r\n\r\n\t\t\tvar d = document;\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.slogan'),\r\n\t\t\t\tthis.getContent('head_claim')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h1'),\r\n\t\t\t\tthis.getContent('slider_hl')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h2 > span'),\r\n\t\t\t\tthis.getContent('slider_sub_hl')\r\n\t\t\t);\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[0],\r\n\t\t\t\tthis.getContent('slider_uvp_1')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[1],\r\n\t\t\t\tthis.getContent('slider_uvp_2')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[2],\r\n\t\t\t\tthis.getContent('slider_uvp_3')\r\n\t\t\t);\r\n\r\n\r\n\t\t\t\/\/DATENEINGABE - - - - -\r\n\r\n\t\t\tif(this.pagetype === 'dateneingabe'){\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .col-head-form > .alert-success'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_notification')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .formular .col-head-form h3'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_intro')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tvar checkIcon = '<i class=\"glyphicon glyphicon-ok\"><\/i> ';\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[0],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_1')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[1],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_2')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[2],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_3')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tif(this.getContent('dateneingabe_cta')){\r\n\t\t\t\t\td.querySelector('#mfw_submit_adressInputStep1').setAttribute('value', this.getContent('dateneingabe_cta'));\r\n\t\t\t\t}\r\n\r\n\t\t\t}\r\n\r\n\t\t}\r\n\r\n    };\r\n\r\n})();\r\n","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}},{"id":50148453,"name":"Variation #1","baseline":0,"traffic_allocation":51,"js":"var wtf = true;","css":"body{\r\n\tborder: 5px solid blue;\r\n}","experiment":{"id":49616615,"status":"running","name":"Headlines with direct Speech","traffic_allocation":100,"audiences_match_type":"any","js":"\/\/https:\/\/solaranlagen.firmen.de\/start_1480.html \r\n\/\/https:\/\/photovoltaikanlagen.firmen.de\/start_1576.html\r\n\r\n(function(){\r\n\r\n    \/* + + + + + + + + + + ArTools + + + + + + + + + + *\/\r\n\r\n\tvar ArTools = {\r\n\r\n\t\tinit: function(){\r\n\t\t\tvar _self = this;\r\n\t\t\twindow.addEventListener('load', function(){\r\n        \t\t_self.runVariationCallback();\r\n        \t}, false);\r\n\t\t},\r\n\r\n        runVariationCallback: function(){\r\n            ArVariation.init();\r\n        },\r\n\r\n\t\tsetCookie: function(name, value, days){\r\n\t\t\tvar expires = \"\";\r\n\t\t\tif(days){\r\n\t\t\t\tvar date = new Date();\r\n\t\t\t\tdate.setTime(date.getTime()+(days*24*60*60*1000));\r\n\t\t\t\texpires = \"; expires=\"+date.toGMTString();\r\n\t\t\t}\r\n\t\t\tdocument.cookie = name+\"=\"+value+expires+\"; path=\/\";\r\n\t\t},\r\n\r\n\t\tgetCookie: function(name){\r\n\t\t\tvar nameEQ = name + \"=\";\r\n\t\t\tvar ca = document.cookie.split(';');\r\n\t\t\tfor(var i=0;i < ca.length;i++) {\r\n\t\t\t\tvar c = ca[i];\r\n\t\t\t\twhile (c.charAt(0)===' '){\r\n\t\t\t\t\tc = c.substring(1,c.length);\r\n\t\t\t\t}\r\n\t\t\t\tif (c.indexOf(nameEQ) === 0){\r\n\t\t\t\t\treturn c.substring(nameEQ.length,c.length);\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\treturn null;\r\n\t\t},\r\n\r\n\t\tremoveCookie: function(name){\r\n  \t\t\tdocument.cookie = name +'=; Path=\/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';\r\n\t\t},\r\n\r\n\t\tgetUrlParameter: function(name, url){\r\n\t\t    if(!url){\r\n\t\t      url = window.location.href;\r\n\t\t    }\r\n\t\t    name = name.replace(\/[\\[\\]]\/g, \"\\\\$&\");\r\n\t\t    var regex = new RegExp(\"[?&]\" + name + \"(=([^&#]*)|&|#|$)\"),\r\n\t\t        results = regex.exec(url);\r\n\t\t    if(!results){\r\n\t\t        return null;\r\n\t\t    }\r\n\t\t    if(!results[2]){\r\n\t\t        return '';\r\n\t\t    }\r\n\t\t    return decodeURIComponent(results[2].replace(\/\\+\/g, \" \"));\r\n\t\t},\r\n\r\n\t};\r\n\r\n\tArTools.init();\r\n\r\n\r\n    \/* + + + + + + + + + + ArVariation + + + + + + + + + + *\/\r\n\r\n    var ArVariation = {\r\n\r\n        init: function(){\r\n\t\t\tthis.addHelpers();\r\n\t\t\tthis.run();\r\n\t\t},\r\n\r\n        addHelpers: function(){\r\n            document.documentElement.classList.add('arExp_001__1');\r\n\t\t\tif(window.location.href.indexOf('datenEingabe') > -1){\r\n\t\t\t\tthis.pagetype = 'dateneingabe';\r\n\t\t       document.documentElement.classList.add('arPage_dateneingabe');\r\n\t\t    }\r\n\t\t\telse{\r\n\t\t\t\tthis.pagetype = 'default';\r\n\t\t\t\tdocument.documentElement.classList.add('arPage_default');\r\n\t\t\t}\r\n        },\r\n\r\n\t\trun: function(){\r\n\t\t\t\/\/General Alternation\r\n\t\t\tthis.alterStepPlz();\r\n\t\t\t\/\/Intent\r\n\t\t\tthis.setIntent();\r\n\t\t\tthis.intentId = this.getIntent();\r\n\t\t\tif(this.intentId){\r\n\t\t\t\tthis.changeElements();\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\t\/* - - - - - - - - - - Variation Functions - - - - - - - - - - *\/\r\n\r\n\t\tgetSubdomain: function(){\r\n\t\t\tvar hostname = window.location.hostname;\r\n\t\t\tvar regexParse = new RegExp('[a-z\\-0-9]{2,63}\\.[a-z\\.]{2,5}$');\r\n        \tvar urlParts = regexParse.exec(hostname);\r\n        \treturn hostname.replace(urlParts[0],'').slice(0, -1);\r\n\t\t},\r\n\r\n\t\tsetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getUrlParameter('i');\r\n\t\t\tif(intentId){\r\n\t\t\t\tArTools.setCookie('userintent', intentId);\r\n\t\t\t\tif(intentId === 'reset'){\r\n\t\t\t\t\tArTools.removeCookie('userintent');\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetIntent: function(){\r\n\t\t\tvar intentId = ArTools.getCookie('userintent');\r\n\t\t\tif(intentId){\r\n\t\t\t\treturn intentId;\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\talterStepPlz: function(){\r\n\t\t\ttry {\r\n\t\t\t\tvar subdomain = this.getSubdomain();\r\n\t\t\t\tvar frageLiClass = 'frage8';\r\n\t\t\t\tif(subdomain === 'photovoltaikanlagen'){\r\n\t\t\t\t\tfrageLiClass = 'frage15';\r\n\t\t\t\t}\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass).classList.add('arStepPlz');\r\n\r\n\t\t\t\tvar plzCta = document.querySelector('li.'+frageLiClass+' .sliderNextWrap > button');\r\n\t\t\t\tplzCta.classList.add('arBtnNextPlz');\r\n\t\t\t\tplzCta.insertAdjacentHTML('afterbegin', '\u00bb ');\r\n\r\n\t\t\t\tvar plzErrorLabel = '<div class=\"arPlzErrorLabel\"><\/div>';\r\n\t\t\t\tdocument.querySelector('li.'+frageLiClass+' .sliderNextWrap').insertAdjacentHTML('afterend', plzErrorLabel);\r\n\r\n\t\t\t\tvar plzInput = document.querySelector('li.'+frageLiClass+' .sliderAnswerWrap input');\r\n\t\t\t\tplzCta.addEventListener('click', function(){\r\n\t\t\t\t\tsetTimeout(function(){\r\n\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '';\r\n\t\t\t\t\t\tif(plzInput.classList.contains('mfwError')){\r\n\t\t\t\t\t\t\tdocument.querySelector('.arPlzErrorLabel').innerHTML = '<span><i class=\"fa fa-info-circle\" aria-hidden=\"true\"><\/i> Bitte geben Sie eine <strong>PLZ<\/strong> an, damit wir <strong>regionale Anbieter<\/strong> f\u00fcr Sie ermitteln k\u00f6nnen.<\/span>';\r\n\t\t\t\t\t\t}\r\n\t\t\t\t\t}, 25);\r\n\t\t\t\t});\r\n\t\t\t} catch(e){\r\n\t\t\t\t\/\/Nothing\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tgetContentArray: function(){\r\n\t\t\tvar subdomain = this.getSubdomain();\r\n\r\n\t\t\tvar content = {\r\n\r\n\t\t\t\t\/\/SOLARANLAGEN\r\n\t\t\t\tsolaranlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Solaranlagen',\r\n\t\t\t\t\t\tslider_hl: 'Solaranlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Mietangebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t\t\/\/PHOTOVOLTAIKANLAGEN\r\n\t\t\t\tphotovoltaikanlagen: [\r\n\t\t\t\t\t\/\/ 1 = Mieten\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Mieten von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage mieten & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Mietangebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Angebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Mietangebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 2 = Kaufen\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\thead_claim: 'Die besten Angebote zum Kauf von Photovoltaikanlagen',\r\n\t\t\t\t\t\tslider_hl: 'PV-Anlage kaufen & unabh\u00e4ngig sein',\r\n\t\t\t\t\t\tslider_sub_hl: 'Angebote vergleichen & sparen!',\r\n\t\t\t\t\t\tslider_uvp_1: 'Kostenlos & unverbindlich',\r\n\t\t\t\t\t\tslider_uvp_2: 'Zeit & Geld sparen',\r\n\t\t\t\t\t\tslider_uvp_3: 'Die besten Angebote vergleichen',\r\n\t\t\t\t\t\tdateneingabe_notification: 'Mehr als 3 passende Angebote gefunden',\r\n\t\t\t\t\t\tdateneingabe_intro: 'Um die besten Anbieter f\u00fcr Ihre individuellen Angebote ermitteln zu k\u00f6nnen, rufen wir Sie schnellstm\u00f6glich an.',\r\n\t\t\t\t\t\tdateneingabe_uvp_1: 'einfach & sicher',\r\n\t\t\t\t\t\tdateneingabe_uvp_2: 'T\u00dcV-gepr\u00fcfter Datenschutz',\r\n\t\t\t\t\t\tdateneingabe_uvp_3: 'Agebote von ausgew\u00e4hlten Fachbetrieben',\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Angebote vergleichen'\r\n\t\t\t\t\t},\r\n\t\t\t\t\t\/\/ 3 = Rechner\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tdateneingabe_cta: 'Jetzt Berechnung starten'\r\n\t\t\t\t\t}\r\n\t\t\t\t],\r\n\r\n\t\t\t};\r\n\r\n\t\t\treturn content[subdomain];\r\n\t\t},\r\n\r\n\t\tgetContent: function(label){\r\n\t\t\tvar cid = this.intentId -1;\r\n\t\t\tvar content = this.getContentArray();\r\n\t\t\tif(content && cid in content){\r\n\t\t\t\treturn content[cid][label];\r\n\t\t\t}\r\n\t\t\treturn false;\r\n\t\t},\r\n\r\n\t\tchangeContent: function(element, content){\r\n\t\t\tif(element && content){\r\n\t\t\t\telement.innerHTML = content;\r\n\t\t\t}\r\n\t\t},\r\n\r\n\t\tchangeElements: function(){\r\n\r\n\t\t\t\/\/GENERELL - - - - -\r\n\r\n\t\t\tvar d = document;\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.slogan'),\r\n\t\t\t\tthis.getContent('head_claim')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h1'),\r\n\t\t\t\tthis.getContent('slider_hl')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelector('.col-headline > h2 > span'),\r\n\t\t\t\tthis.getContent('slider_sub_hl')\r\n\t\t\t);\r\n\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[0],\r\n\t\t\t\tthis.getContent('slider_uvp_1')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[1],\r\n\t\t\t\tthis.getContent('slider_uvp_2')\r\n\t\t\t);\r\n\t\t\tthis.changeContent(\r\n\t\t\t\td.querySelectorAll('.liste-box .liste-top')[2],\r\n\t\t\t\tthis.getContent('slider_uvp_3')\r\n\t\t\t);\r\n\r\n\r\n\t\t\t\/\/DATENEINGABE - - - - -\r\n\r\n\t\t\tif(this.pagetype === 'dateneingabe'){\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .col-head-form > .alert-success'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_notification')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelector('.arPage_dateneingabe .formular .col-head-form h3'),\r\n\t\t\t\t\tthis.getContent('dateneingabe_intro')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tvar checkIcon = '<i class=\"glyphicon glyphicon-ok\"><\/i> ';\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[0],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_1')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[1],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_2')\r\n\t\t\t\t);\r\n\t\t\t\tthis.changeContent(\r\n\t\t\t\t\td.querySelectorAll('.arPage_dateneingabe .formular .liste > li')[2],\r\n\t\t\t\t\tcheckIcon + this.getContent('dateneingabe_uvp_3')\r\n\t\t\t\t);\r\n\r\n\t\t\t\tif(this.getContent('dateneingabe_cta')){\r\n\t\t\t\t\td.querySelector('#mfw_submit_adressInputStep1').setAttribute('value', this.getContent('dateneingabe_cta'));\r\n\t\t\t\t}\r\n\r\n\t\t\t}\r\n\r\n\t\t}\r\n\r\n    };\r\n\r\n})();\r\n","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}}],"pages":[{"id":59968434,"name":"Index","rules_js":"return window.testVar;","pivot":{"experiment_id":49616615,"page_id":59968434},"urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]},{"id":67848484,"name":"Step 2","rules_js":"","pivot":{"experiment_id":49616615,"page_id":67848484},"urltargetings":[{"id":26,"type":"include","url":"http:\/\/local.test.abpro.io\/step_2.php","url_type":"simple"}]},{"id":29218726,"name":"Dummy","rules_js":"","pivot":{"experiment_id":49616615,"page_id":29218726},"urltargetings":[{"id":21,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":22,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":23,"type":"exclude","url":"https:\/\/companywithbrain.com\/product\/out-of-stock","url_type":"exact"},{"id":24,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":25,"type":"exclude","url":"absimple.io","url_type":"substring"}]}],"goals":[{"id":80660831,"type":"pageview","counting_method":"every","css_selector":null,"name":"Visited Index","api_name":null,"rules_js":null,"pivot":{"experiment_id":49616615,"goal_id":80660831},"page":{"id":59968434,"name":"Index","rules_js":"return window.testVar;","urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}},{"id":63125054,"type":"click","counting_method":"every","css_selector":".pricing-header > h1","name":"Click on Headline","api_name":null,"rules_js":null,"pivot":{"experiment_id":49616615,"goal_id":63125054},"page":{"id":59968434,"name":"Index","rules_js":"return window.testVar;","urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}},{"id":39474237,"type":"click","counting_method":"every","css_selector":"#linkStep2","name":"Click Link \"Step 2\"","api_name":null,"rules_js":null,"pivot":{"experiment_id":49616615,"goal_id":39474237},"page":{"id":59968434,"name":"Index","rules_js":"return window.testVar;","urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}},{"id":61892330,"type":"pageview","counting_method":"every","css_selector":null,"name":"Visited Step 2","api_name":null,"rules_js":null,"pivot":{"experiment_id":49616615,"goal_id":61892330},"page":{"id":67848484,"name":"Step 2","rules_js":"","urltargetings":[{"id":26,"type":"include","url":"http:\/\/local.test.abpro.io\/step_2.php","url_type":"simple"}]}},{"id":99350290,"type":"custom","counting_method":"every","css_selector":null,"name":"Test Custom Goal","api_name":"test-custom-goal","rules_js":null,"pivot":{"experiment_id":49616615,"goal_id":99350290},"page":null}],"project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}},{"id":62108254,"status":"running","name":"Exclusion Experiment 1","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp1');","css":"","audiences":[{"id":49788342,"name":"Desktop Users","rules_js":"if(User.browser.desktop){\r\n  return true;\r\n}\r\n"}],"variations":[{"id":73784313,"name":"Original","baseline":1,"traffic_allocation":50,"js":"","css":"","experiment":{"id":62108254,"status":"running","name":"Exclusion Experiment 1","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp1');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}},{"id":13455842,"name":"Variation #1","baseline":0,"traffic_allocation":50,"js":"","css":"","experiment":{"id":62108254,"status":"running","name":"Exclusion Experiment 1","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp1');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}}],"pages":[{"id":59968434,"name":"Index","rules_js":"return window.testVar;","pivot":{"experiment_id":62108254,"page_id":59968434},"urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}],"goals":[],"project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}},{"id":92254361,"status":"running","name":"Exclusion Experiment 2","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp2');","css":"","audiences":[{"id":49788342,"name":"Desktop Users","rules_js":"if(User.browser.desktop){\r\n  return true;\r\n}\r\n"}],"variations":[{"id":62766901,"name":"Original","baseline":1,"traffic_allocation":50,"js":"","css":"","experiment":{"id":92254361,"status":"running","name":"Exclusion Experiment 2","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp2');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}},{"id":99369215,"name":"Variation #1","baseline":0,"traffic_allocation":50,"js":"","css":"","experiment":{"id":92254361,"status":"running","name":"Exclusion Experiment 2","traffic_allocation":100,"audiences_match_type":"all","js":"$('h1').append(' #ExclExp2');","css":"","project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}}],"pages":[{"id":59968434,"name":"Index","rules_js":"return window.testVar;","pivot":{"experiment_id":92254361,"page_id":59968434},"urltargetings":[{"id":1,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":2,"type":"include","url":"http:\/\/google.de","url_type":"simple"},{"id":3,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":4,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":5,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":6,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":7,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":8,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":9,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":10,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":11,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":12,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"},{"id":13,"type":"include","url":"http:\/\/local.test.abpro.io\/","url_type":"simple"}]}],"goals":[],"project":{"id":22732137,"name":"companywithbrain.com","debug":1,"adhere_dnt":0,"max_cookie_lifetime":365,"jquery_include":"3.3.1_slim","use_antiflicker":0,"snippet_revision":677,"js":"\/\/window.$(function() {\r\n\/\/\talert(\"JS ready\");\r\n\/\/})\r\n\/\/https:\/\/s3.eu-central-1.amazonaws.com\/assets.abpro.io\/22732137\/oQGOAxJacpXlZykd7RIQAVH0DZjJiHV07cFu9Adv.png","css":"body{\r\n\tborder: 5px solid green;\r\n}"}}],"exclusion_groups":[{"id":58120900,"name":"Exclusion Group 1","exclusion_group_entries":[{"experiment_id":62108254,"traffic":50},{"experiment_id":92254361,"traffic":25}]},{"id":58583111,"name":"Exclusion Group 2","exclusion_group_entries":[{"experiment_id":92254361,"traffic":50},{"experiment_id":16552854,"traffic":25}]}],"custom_integrations":[],"default_integrations":[{"id":4,"name":"Webtrekk","help_url":"https:\/\/support.webtrekk.com\/hc\/de\/article_attachments\/115007805409\/Webtrekk-Implementation-Guidelines-v6-EN.PDF","logo":"webtrekk.png","used_hook":"experimentViewed","js":"window._ti = window._ti || {};\r\nwindow._ti['contentGroup'] = {\r\n\t1: experimentName,\r\n\t2: variationName\r\n};","pivot":{"project_id":22732137,"default_integration_id":4}},{"id":5,"name":"ContentSquare","help_url":"https:\/\/docs.contentsquare.com\/uxa-en\/#sending-custom-vars","logo":"contentsquare.png","used_hook":"experimentViewed","js":"window._uxa = window._uxa || [];\r\nwindow._uxa.push(['setCustomVariable', 20, experimentName, variationName]);","pivot":{"project_id":22732137,"default_integration_id":5}}]});
	  				...
	  				Tools.poll(
	  					function(){
	  			  		...
	  			  		function(){
	  			  			...
	  			  			if(!Script.isPreview){
	  			  				if(!experiment.passesExclusionGroups()){
	  			  					return;
	  			  				}
	  			  			}
